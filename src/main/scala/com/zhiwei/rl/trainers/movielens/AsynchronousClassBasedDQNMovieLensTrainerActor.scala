...
import com.zhiwei.rl.agents.movielens.AsynchronousClassBasedDQNMovieLensAgentWorkerActor
...
object AsynchronousClassBasedDQNMovieLensTrainerActor{
  ...
  def props(
    ...
    Props(
      new AsynchronousClassBasedDQNMovieLensTrainerActor(
        clusterConfig,
        recSysConfig,
        replayQueueConfig,
        dataSetMacro,
...
class AsynchronousClassBasedDQNMovieLensTrainerActor(
                                          clusterConfig: ClusterConfigT,
                                          recSysConfig: RecSysConfigT,
                                          replayQueueConfig: ReplayQueueConfigT,
                                          dataSetMacro: MovieLensDataSetMacro,
  ...
  val trainerName = "AsynchronousClassBasedDQNMovieLensTrainer"
  ...
  val workerRouterActorRef: ActorRef = context.actorOf(
    BalancingPool(numWorker).props(
      AsynchronousClassBasedDQNMovieLensAgentWorkerActor.props(
        clusterConfig,
        onlineNetwork,
        rLConfig,
        recSysConfig,
    ...
    ,
    "AsynchronousClassBasedDQNMAgentRouter"
  )
}